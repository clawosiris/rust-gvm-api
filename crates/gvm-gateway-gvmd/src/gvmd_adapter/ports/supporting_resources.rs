        ...
        let (items, total) = match kind {
            SecInfoKind::Cve => {
                let response = client
                    ...
                    .await
                    .map_err(map_gvm_error)?;
                let parsed = GetCvesResponse::from_response(&response).map_err(map_parse_error)?;
                let items = parsed
                    .items
                    .into_iter()
                    .map(|item| SecInfoItem {
                        id: item.id,
                        name: item.name,
                    })
                    .collect::<Vec<_>>();
            ...
            SecInfoKind::Cpe => {
                let response = client
                    ...
                    .await
                    .map_err(map_gvm_error)?;
                let parsed = GetCpesResponse::from_response(&response).map_err(map_parse_error)?;
                let items = parsed
                    .items
                    .into_iter()
                    .map(|item| SecInfoItem {
            ...
            SecInfoKind::CertBundAdvisory => {
                let response = client
                    .lock()
                    .await?
                    .call(get_cert_bund_advisories(opts))
                    .await
                    .map_err(map_gvm_error)?;
            ...
            SecInfoKind::DfnCertAdvisory => {
                let response = client
                    .lock()
                    .await?
                    .call(get_dfn_cert_advisories(opts))
                    .await
                    .map_err(map_gvm_error)?;
                let parsed = GetDfnCertAdvisoriesResponse::from_response(&response)
                    .map_err(map_parse_error)?;
                let items = parsed
                    .items
                    .into_iter()
