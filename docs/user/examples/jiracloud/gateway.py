...
class GvmClient:
    ...
    def get_recent_reports(self, now_utc: datetime) -> list[dict[str, Any]]:
        cutoff = now_utc - timedelta(hours=self.config.jira_lookback_hours)
    ...
    def get_report_results(self, report_id: str) -> list[dict[str, Any]]:
        ...
        return [result for result in results if result_severity(result) > self.config.jira_min_severity]
...
def result_severity(result: dict[str, Any]) -> float:
    ...
    try:
        ...
        raise IntegrationError(f"Result {result.get('id')} has non-numeric severity {severity!r}.") from exc
